import 'package:flutter/material.dart';

const TextStyle heading1 = TextStyle(
  fontSize: 30,
  fontWeight: FontWeight.w500,
);

const TextStyle heading1Light = TextStyle(
  fontSize: 30,
  fontWeight: FontWeight.w400,
);

const TextStyle heading1Bold = TextStyle(
  fontSize: 30,
  fontWeight: FontWeight.w700,
);

const TextStyle heading2 = TextStyle(
  fontSize: 24,
  fontWeight: FontWeight.w500,
);

const TextStyle heading2Light = TextStyle(
  fontSize: 24,
  fontWeight: FontWeight.w400,
);

const TextStyle heading2Bold = TextStyle(
  fontSize: 24,
  fontWeight: FontWeight.w700,
);

const TextStyle heading3 = TextStyle(
  fontSize: 22,
  fontWeight: FontWeight.w500,
);

const TextStyle heading3Light = TextStyle(
  fontSize: 22,
  fontWeight: FontWeight.w400,
);

const TextStyle heading3Bold = TextStyle(
  fontSize: 22,
  fontWeight: FontWeight.w700,
);

const TextStyle heading20Bold = TextStyle(
  fontSize: 20,
  fontWeight: FontWeight.w700,
);

const TextStyle subtitleBig = TextStyle(
  fontSize: 18,
  fontWeight: FontWeight.w500,
);

const TextStyle subtitleBigLight = TextStyle(
  fontSize: 18,
  fontWeight: FontWeight.w400,
);

const TextStyle subtitleBigBold = TextStyle(
  fontSize: 18,
  fontWeight: FontWeight.w700,
);

const TextStyle subtitle = TextStyle(
  fontSize: 16,
  fontWeight: FontWeight.w500,
);

const TextStyle subtitleLight = TextStyle(
  fontSize: 16,
  fontWeight: FontWeight.w400,
);

const TextStyle subtitleBold = TextStyle(
  fontSize: 16,
  fontWeight: FontWeight.w700,
);

const TextStyle body14 = TextStyle(
  fontSize: 14,
  fontWeight: FontWeight.w500,
);

const TextStyle body14Light = TextStyle(
  fontSize: 14,
  fontWeight: FontWeight.w400,
);

const TextStyle body14Bold = TextStyle(
  fontSize: 14,
  fontWeight: FontWeight.w700,
);

const TextStyle caption = TextStyle(
  fontSize: 12,
  fontWeight: FontWeight.w500,
);

const TextStyle captionLight = TextStyle(
  fontSize: 12,
  fontWeight: FontWeight.w400,
);

const TextStyle captionBold = TextStyle(
  fontSize: 12,
  fontWeight: FontWeight.w700,
);

const TextStyle overline = TextStyle(
  fontSize: 10,
  fontWeight: FontWeight.w500,
);

const TextStyle overlineLight = TextStyle(
  fontSize: 10,
  fontWeight: FontWeight.w400,
);

const TextStyle overlineBold = TextStyle(
  fontSize: 10,
  fontWeight: FontWeight.w700,
);


